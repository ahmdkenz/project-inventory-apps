<template>
  <div class="flex h-screen">
    ...
    <div class="flex-1 flex flex-col transition-all duration-300 ease-in-out lg:ml-64" id="main-content">
      ...
      <div
        v-if="showAlert"
        class="fixed top-20 right-6 py-3 px-6 rounded-xl shadow-lg z-50 text-white flex items-center"
        ...
        <div class="lg:grid lg:grid-cols-12 lg:gap-8">
          ...
          <div class="lg:col-span-8 space-y-6">
            <div class="flex items-center justify-between mb-4 lg:hidden">
              ...
              <router-link to="/admin/purchase-orders" class="bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 font-medium py-2 px-3 rounded-lg text-sm">Kembali</router-link>
            </div>
              ...
              <form @submit.prevent="addItem" class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6 items-end p-4 bg-slate-50 rounded-lg border border-slate-100">
                <div class="md:col-span-5">
                  ...
                  <select v-model="newItem.barang_id" id="item_select" class="w-full rounded-lg border-gray-300 shadow-sm p-2.5 text-sm">
                    ...
                    <option v-for="barang in barangList" :key="barang.id" :value="barang.id" :data-nama="barang.nama">{{ barang.kode }} - {{ barang.nama }}</option>
                  </select>
                </div>
                <div class="md:col-span-3">
                  <label for="item_qty" class="block text-sm font-medium text-gray-700 mb-1">Jumlah</label>
                  <input v-model.number="newItem.qty" type="number" id="item_qty" min="1" class="w-full rounded-lg border-gray-300 shadow-sm p-2.5 text-sm">
                </div>
                <div class="md:col-span-3">
                  ...
                  <div class="relative">
                    ...
                    <input v-model.number="newItem.price" type="number" id="item_price" min="0" placeholder="0" class="w-full rounded-lg border-gray-300 shadow-sm p-2.5 pl-8 text-sm">
                  </div>
                </div>
                <div class="md:col-span-1">
                  <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-3 rounded-lg flex items-center justify-center transition-all duration-300">
                ...
                <table class="w-full min-w-max">
                  ...
                  <tbody class="divide-y divide-slate-200">
                    <tr v-for="(item, index) in formData.items" :key="index" class="hover:bg-slate-50 transition-colors duration-150">
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-600">{{ item.qty }}</td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-600">{{ formatCurrency(item.price) }}</td>
                    ...
                    <tr v-if="formData.items.length === 0">
                      <td colspan="5" class="px-6 py-10 text-center text-sm text-gray-500">
                        ...
                        <p class="text-xs text-gray-400">Silakan pilih barang dan klik tambah.</p>
                      </td>
                    </tr>
                  </tbody>
                  <tfoot v-show="formData.items.length > 0" class="bg-slate-50 border-t border-slate-200">
                    <tr>
            ...
            <div class="lg:sticky lg:top-8 space-y-6">
              <div class="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    <select id="type" v-model="formData.type" class="w-full rounded-lg border-gray-300 shadow-sm p-3">
                  ...
                  <div>
                    ...
                    <input v-model="formData.source" id="source" type="text" class="w-full rounded-lg border-gray-300 shadow-sm p-3" placeholder="Cth: Retur Divisi IT, Hibah..." />
                  </div>

                  <div>
                    <label for="receive_date" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Diterima</label>
                    <input v-model="formData.receive_date" id="receive_date" type="date" class="w-full rounded-lg border-gray-300 shadow-sm p-3" />
                  </div>

                  <div>
                    <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Catatan (Penting)</label>
                    <textarea v-model="formData.notes" id="notes" rows="3" class="w-full rounded-lg border-gray-300 shadow-sm p-3" placeholder="Jelaskan alasan penerimaan ini secara rinci..."></textarea>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-white p-6 rounded-xl shadow-lg border border-slate-200">
                ...
                <div class="flex flex-col gap-3">
                  <router-link to="/admin/purchase-orders" class="w-full text-center bg-white hover:bg-slate-100 border border-slate-300 text-slate-700 font-medium py-3 px-4 rounded-lg">Batal</router-link>
                  <button type="button" @click="submitReceipt" class="w-full text-center bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg shadow-lg">Simpan Penerimaan</button>
                </div>
              </div>
            </div>
          </div>
...
const submitReceipt = async () => {
  ...
  try {
    const token = localStorage.getItem('token')
    const response = await axios.post('http://localhost:8000/api/admin/non-po/receipt', formData.value, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Accept': 'application/json',
    ...
    if (response.data.success) {
      ...
      setTimeout(() => {
        router.push(`/admin/non-po/receipt/${response.data.data.id}/print`)
      }, 1500)
    } else {
