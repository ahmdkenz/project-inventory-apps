<template>
  <div class="flex h-screen bg-gray-100">
        ...
        <div v-else-if="issue">
          ...
          <div class="flex items-center justify-between mb-6 print-hidden">
            ...
            <div class="flex space-x-3">
              ...
              <router-link 
                :to="`/admin/non-po/issue/${issue.id}/print`"
          ...
          <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div class="bg-white p-8 rounded-lg shadow-lg border border-gray-200">
                ...
                <div class="flex justify-between items-start pb-6 border-b-2 border-gray-900">
                  <div>
                    ...
                    <p class="text-lg font-semibold text-blue-600">{{ issue.no_dokumen }}</p>
                    <span :class="getStatusBadgeClass(issue.status)" class="inline-block px-3 py-1 text-xs font-medium rounded-full mt-2">
                ...
                <div class="grid grid-cols-2 gap-8 my-6">
                  <div>
                    ...
                    <p class="text-lg font-bold text-gray-900">{{ issue.recipient }}</p>
                    <p v-if="issue.notes" class="text-sm text-gray-600">{{ issue.notes }}</p>
                  </div>
                  <div class="text-right">
                    <div class="mb-2">
                      <span class="text-sm font-semibold text-gray-500 uppercase">Tanggal Dikeluarkan: </span>
                      <span class="text-sm font-medium text-gray-800">{{ formatDate(issue.issue_date) }}</span>
                    </div>
                    <div>
                      <span class="text-sm font-semibold text-gray-500 uppercase">Dicatat Oleh: </span>
                      <span class="text-sm font-medium text-gray-800">{{ issue.creator?.name || '-' }}</span>
                    </div>
                  </div>
                </div>
                ...
                <div class="overflow-x-auto border rounded-lg mb-6">
                  <table class="w-full min-w-max">
                    ...
                    <tbody class="divide-y divide-gray-200">
                      <tr v-for="(item, index) in issue.items" :key="index">
                    ...
                    <tfoot class="bg-gray-50 font-medium">
                      <tr>
                        <td colspan="3" rowspan="3" class="px-6 py-3 align-top">
                          ...
                          <p class="text-sm text-gray-600">{{ issue.notes || '-' }}</p>
                        </td>
                        <td class="px-6 py-3 text-right text-sm text-gray-700 uppercase">Subtotal</td>
                        <td class="px-6 py-3 text-right text-sm text-gray-900">{{ formatCurrency(issue.subtotal || 0) }}</td>
                      </tr>
                      <tr>
                      ...
                      <tr class="text-base font-semibold">
                        ...
                        <td class="px-6 py-4 text-right text-gray-900">{{ formatCurrency(issue.total || 0) }}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
                ...
                <div class="grid grid-cols-2 gap-8 pt-6 border-t border-gray-200">
                  <div>
                    ...
                    <p class="text-sm font-medium text-gray-900 border-t border-gray-400 pt-1">({{ issue.creator?.name || 'Staff' }})</p>
                    <p class="text-xs text-gray-500">Staff Inventori</p>
                  </div>
                  <div class="text-center">
                    <p class="text-sm text-gray-700 mb-16">Disetujui oleh,</p>
                    <p class="text-sm font-medium text-gray-900 border-t border-gray-400 pt-1">({{ issue.approver?.name || 'Admin' }})</p>
            ...
            <div class="lg:col-span-4 space-y-6 print-hidden">
              ...
              <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                ...
                <div class="space-y-3">
                  <div>
                    ...
                    <dd class="text-base text-gray-900 font-medium">{{ issue.no_dokumen }}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500">Status</dt>
                    <dd>
                      <span :class="getStatusBadgeClass(issue.status)" class="inline-block px-3 py-1 text-xs font-medium rounded-full">
                        {{ getStatusText(issue.status) }}
                      </span>
                    </dd>
                  </div>
                  <div>
                    ...
                    <dd class="text-base text-gray-900 font-medium">{{ formatDate(issue.issue_date) }}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500">Dicatat Oleh</dt>
                    <dd class="text-base text-gray-900 font-medium">{{ issue.creator?.name || '-' }}</dd>
                  </div>
                </div>
              </div>
              ...
              <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                ...
                <div v-if="issue.status === 'pending'" class="space-y-3">
                ...
                <div v-else-if="issue.status === 'completed'" class="text-center py-4">
                  <svg class="mx-auto h-12 w-12 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p class="mt-2 text-sm text-gray-600">Pengeluaran sudah disetujui</p>
                  <p class="text-xs text-gray-500">{{ issue.approved_at ? formatDate(issue.approved_at) : '-' }}</p>
                </div>
                <div v-else-if="issue.status === 'rejected'" class="text-center py-4">
                  <svg class="mx-auto h-12 w-12 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p class="mt-2 text-sm text-gray-600">Pengeluaran ditolak</p>
                  <p class="text-xs text-gray-500">Alasan: {{ issue.reject_reason || '-' }}</p>
                </div>
              </div>
              ...
              <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                ...
                <ol class="relative border-l border-gray-300">
                  <li class="ml-4 mb-4">
                    ...
                    <time class="mb-1 text-sm font-normal leading-none text-gray-500">{{ formatDate(issue.created_at) }}</time>
                    <p class="text-sm font-medium text-gray-900">Pengeluaran dibuat</p>
                    <p class="text-sm text-gray-600">Oleh {{ issue.creator?.name || 'Staff' }}</p>
                  </li>
                  <li v-if="issue.approved_at && issue.status === 'completed'" class="ml-4">
                    <div class="absolute w-3 h-3 bg-green-500 rounded-full -left-1.5 border border-white"></div>
                    <time class="mb-1 text-sm font-normal leading-none text-gray-500">{{ formatDate(issue.approved_at) }}</time>
                    <p class="text-sm font-medium text-gray-900">Pengeluaran disetujui</p>
                    <p class="text-sm text-gray-600">Oleh {{ issue.approver?.name || 'Admin' }}</p>
                  </li>
                  <li v-if="issue.approved_at && issue.status === 'rejected'" class="ml-4">
                    <div class="absolute w-3 h-3 bg-red-500 rounded-full -left-1.5 border border-white"></div>
                    <time class="mb-1 text-sm font-normal leading-none text-gray-500">{{ formatDate(issue.approved_at) }}</time>
                    <p class="text-sm font-medium text-gray-900">Pengeluaran ditolak</p>
                    <p class="text-sm text-gray-600">Alasan: {{ issue.reject_reason || '-' }}</p>
                  </li>
                </ol>
              </div>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all">
        <div class="p-6 border-b border-gray-200">
          <h3 class="text-lg font-medium leading-6 text-gray-900">Setujui Pengeluaran Non-SO</h3>
        </div>
        <div class="p-6">
          <p class="text-sm text-gray-600">
            Apakah Anda yakin ingin menyetujui pengeluaran <strong>{{ issue?.no_dokumen }}</strong>?
          </p>
    ...
    >
      <div class="bg-white rounded-lg shadow-xl w-full max-w-md transform transition-all">
        <div class="p-6 border-b border-gray-200">
          <h3 class="text-lg font-medium leading-6 text-gray-900">Tolak Pengeluaran Non-SO</h3>
        </div>
        <div class="p-6">
          <p class="text-sm text-gray-600 mb-4">
            Anda akan menolak pengeluaran <strong>{{ issue?.no_dokumen }}</strong>.
...
const goBack = () => {
  router.push('/admin/outgoing-items')
}
...
const confirmApprove = async () => {
  ...
  loading.value = true
  try {
    const response = await api.post(`/admin/non-po/issue/${issue.value.id}/approve`)
    if (response.data.success) {
      showMessage('Pengeluaran berhasil disetujui', false)
      closeApproveModal()
      await fetchIssue()
    }
  } catch (error: any) {
    console.error('Error approving issue:', error)
    showMessage(error.response?.data?.message || 'Gagal menyetujui pengeluaran', true)
  } finally {
    loading.value = false
  }
}
...
const confirmReject = async () => {
  ...
  loading.value = true
  try {
    const response = await api.post(`/admin/non-po/issue/${issue.value.id}/reject`, {
      reason: rejectReason.value
    })
    if (response.data.success) {
      showMessage('Pengeluaran berhasil ditolak', false)
      closeRejectModal()
      await fetchIssue()
    }
  } catch (error: any) {
    console.error('Error rejecting issue:', error)
    showMessage(error.response?.data?.message || 'Gagal menolak pengeluaran', true)
  } finally {
    loading.value = false
  }
}
...
const fetchIssue = async () => {
  ...
  try {
    ...
    const response = await api.get(`/admin/non-po/issue/${id}`)
    ...
    showMessage('Gagal memuat data pengeluaran', true)
    setTimeout(() => router.push('/admin/outgoing-items'), 2000)
  } finally {
    loading.value = false
  }
}
