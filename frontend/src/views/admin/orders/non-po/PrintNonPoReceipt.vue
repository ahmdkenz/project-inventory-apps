<template>
  <div class="print-container">
    ...
    <div v-else-if="receipt" id="printable-area">
      <div class="bg-white p-8 md:p-12">
        ...
        <div class="grid grid-cols-2 gap-8 mb-8">
          ...
          <div>
            ...
            <p class="text-gray-700"><span class="font-medium">Tanggal Diterima:</span> {{ formatDate(receipt.approved_at || receipt.receive_date || new Date()) }}</p>
            <p class="text-gray-700"><span class="font-medium">Dicatat Oleh:</span> {{ receipt.receiver?.name || receipt.approver?.name || receipt.creator?.name || 'Admin' }}</p>
            <p class="text-gray-700"><span class="font-medium">No. Surat Jalan:</span> {{ receipt.no_surat_jalan || '-' }}</p>
          </div>
        </div>
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="(item, index) in receipt.items" :key="index">
                <td class="px-4 py-4 text-sm font-medium text-gray-900">
                  <div>{{ item.nama_barang || item.barang?.nama || '-' }}</div>
                  <div class="text-xs text-gray-500">{{ item.kode_barang || item.barang?.kode || '' }}</div>
                </td>
                <td class="px-4 py-4 text-sm font-bold text-gray-900 text-center">{{ item.qty || 0 }}</td>
        ...
        <div class="grid grid-cols-3 gap-8 pt-12">
          ...
          <div class="text-center">
            ...
            <div class="border-t border-gray-400 pt-1">
              <p class="text-sm font-semibold text-gray-900">{{ receipt.receiver?.name || receipt.processor?.name || receipt.creator?.name || '..............................' }}</p>
...
const fetchPrintData = async () => {
  ...
  try {
    ...
    const response = await axios.get(
      `http://localhost:8000/api/admin/non-po/receipt/${id}/print`,
      {
        headers: {
