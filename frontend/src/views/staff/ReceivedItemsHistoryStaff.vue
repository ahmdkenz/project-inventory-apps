<template>
  <div class="flex h-screen">
    ...
    <aside class="fixed top-0 left-0 h-full w-64 bg-gray-900 text-gray-300 shadow-lg z-30 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out" id="sidebar">
      ...
      <!-- Menu Navigasi Staff -->
      <StaffNavigation :current-path="$route.path"></StaffNavigation>
      
      <!-- User Info di Bawah Sidebar -->
      <div class="absolute bottom-0 left-0 w-full p-4 border-t border-gray-700">
        <div class="flex items-center space-x-3">
          <img class="h-10 w-10 rounded-full" src="https://placehold.co/100x100/EBF8FF/3182CE?text=S" alt="Avatar Pengguna">
          <div>
              ...
              <tbody class="divide-y divide-gray-200">
                <tr v-for="item in receivedItems" :key="item.id">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                    <router-link
                      v-if="item.tipe === 'po'"
                      :to="`/staff/purchase-orders/${item.original_id}/print`"
